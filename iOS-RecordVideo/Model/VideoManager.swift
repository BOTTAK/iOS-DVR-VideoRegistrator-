...
class VideoManager: CameraFrameViewController {

    static var instance = VideoManager()
    
    let outputSize = CGSize(width: 1920, height: 1280)
    let imagesPerSecond: TimeInterval = 5
    var selectedPhotosArray = [UIImage]()
    var selected: CameraFrameViewController!
    ...
    func buildVideoFromImageArray(_ imageArray: [UIImage]) {
        selectedPhotosArray = imageArray
        
        imageArrayToVideoURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/temp/video1.MP4")
    ...
    func exportVideoWithAnimation() {
        ...
        for image in 0..<5 {
            imgarray.append(UIImage(named: "\(image + 1).PNG")!)
        ...
        let animatedVideoURL = NSURL(fileURLWithPath: NSHomeDirectory() + "/Documents/temp/video2.mp4")
